...
public class GenericListTest {
	...
	public static void main(String[] args) {
		...
		Product reebook = new Product("S002", "리복 로얄 테크큐 티", 42000, 20);
		...
		// 1. 선언 : Product 만 저장하는 List 로 선언
		List<Product> products;
		
		// 2. 초기화 : Product 만 저장하는 ArrayList로 초기화
		...
		// (1) 리스트에 아이템 추가 add();
		products.add(adidas);
		products.add(reebook);
		products.add(nike);
		products.add(crocs);
		products.add(birkenstock);
		
		// (2) 아이템을 추출할 때 바로 Generic 적용 타입으로
		//     꺼낼 수 있다.
